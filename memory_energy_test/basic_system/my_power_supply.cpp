...
class MyPowerSupply : public vp::Component
{

public:
    MyPowerSupply(vp::ComponentConf &config);
...
MyPowerSupply::MyPowerSupply(vp::ComponentConf &config)
    : vp::Component(config)
{
    this->input_itf.set_req_meth(&MyPowerSupply::handle_req);
...
vp::IoReqStatus MyPowerSupply::handle_req(vp::Block *__this, vp::IoReq *req)
{
    MyPowerSupply *_this = (MyPowerSupply *)__this;
...
void MyPowerSupply::reset(bool active)
{
    if (active)
    {
...
{
    return new MyPowerSupply(config);
}
